...
class Garde_geo extends Component{
  constructor(){
    super()
    this.state={
      lat:null,
  ...
  onMarkerPress=(element)=>{
   ...
   this.setState({
    ...
    desLongitude:lo
      ...
      getPlaces=()=>{
        const url=this.getUrlWithParameters(this.state.lat,this.state.long,20000,'pharmacy','AIzaSyAQ4Udmdmd60avwA2FGIPdu39WBtoFmefg')  
      fetch(url)
      .then((data)=>data.json())
      .then((res)=>{
        const arrayMarker=[];
